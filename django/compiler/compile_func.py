...
def compile_func(lang, code, inputs):
    ...
    if cmp.returncode != 0:
        ...
        try:
            ...
            # inputs = 5
            exec_command = f"sh -c \"echo {inputs} | python /{filename}.py\""
            res = subprocess.run(f"docker exec {cont_name} {exec_command}", capture_output=True, text=True)
            # res = subprocess.run(f"docker exec {cont_name} {exe}", capture_output=True, text=True)
