...
def submit_it(request, prob_id, lang, code):
    ...
    if cmp.returncode != 0:
        ...
        if output == testcase.output:
            print("Accepted")
            print("Required output : {}, Your output: {}", testcase.output, output)
            submission = Submission(problem=problem, user= request.user ,verdict='Accepted', time_stamp=datetime.datetime.now())
            submission.save()
